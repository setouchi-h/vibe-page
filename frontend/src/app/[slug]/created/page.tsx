...
export default async function CreatedPage({ params, searchParams }: PageProps) {
	const { slug } = await params;
	const { key } = await searchParams;

	if (!isValidSlug(slug) || !key) {
		return <Message title="公開URLを表示できません" message="URLが正しくありません。" />;
	}
	...
	return (
		<main className="mx-auto grid min-h-screen w-full max-w-[920px] content-start gap-6 px-4 py-10 sm:px-6 sm:py-16">
			<header className="grid gap-2">
				...
				<h1 className="text-4xl font-bold leading-tight text-slate-950 sm:text-5xl">公開しました</h1>
			</header>
			...
			<section className="grid gap-5 rounded-lg border border-slate-200 bg-white p-5 shadow-xl shadow-slate-200/60 sm:p-6">
				<p className="rounded-md border-l-4 border-teal-700 bg-teal-50 px-4 py-3 text-sm font-medium text-teal-950">
					編集URLは再表示できません。保存してください。
				</p>
				<CopyField id="public-url" label="公開URL" value={publicUrl} />
				<CopyField id="edit-url" label="編集URL" value={editUrl} />
...
function CopyField({ id, label, value }: { id: string; label: string; value: string }) {
	return (
		<div className="grid gap-2">
			<label className="text-sm font-bold text-slate-900" htmlFor={id}>
				{label}
			</label>
			<input
